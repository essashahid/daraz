...
customerRouter.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!match) {
      console.log('Password mismatch for user', customer);
      return res.status(401).json({ status: "error", message: "Invalid credentials" });
    }

    // Include user's name, email, and role in the response
    // print role
    console.log('Login successful for user', customer.role);
    res.status(200).json({ 
      status: "success", 
...
customerRouter.post("/create-customer", async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      customerId: savedCustomer._id // Returning the ID of the newly created customer
    });
  } catch (err) {
    console.log(err);
